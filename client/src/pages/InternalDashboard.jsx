...
export default function InternalDashboard() {
  ...
  useEffect(() => {
    get("/dashboards/internal")
      .then(setSummary)
      .catch((e) => setErr(e.message));
    get("/assignments")
      .then(setTasks)
      .catch((e) => setErr(e.message));
    get(`/meetings?from=${today}`)
  ...
  const derived = useMemo(() => {
    ...
    for (let i = 5; i >= 0; i--) {
      ...
      cvcC.push(
        tasks.filter((t) => ymd(t.start_date).slice(0, 7) === key).length,
      );
      cvcD.push(
        tasks.filter(
          (t) =>
            t.status === "Completed" && ymd(t.due_date).slice(0, 7) === key,
        ).length,
      );
    }
    ...
    const upcoming = open
      .filter((t) => t.due_date)
      .sort((a, b) => (ymd(a.due_date) < ymd(b.due_date) ? -1 : 1))
      .slice(0, 5);
  ...
  return (
    <>
        ...
        >
          <div>
            ...
            <div style={{ color: "#B9C6E2", fontSize: 12.5 }}>
              {total - done} open ·{" "}
              {overdue ? (
                <span style={{ color: "#FF9B92" }}>{overdue} overdue</span>
              ) : (
                "nothing overdue"
              )}
              {" · "}
              {Number(kpi.in_flight) || 0} in progress ·{" "}
              {meetings.filter((m) => ymd(m.meeting_date) === today).length}{" "}
              meetings today
            </div>
          </div>
          <div style={{ display: "flex", gap: 8 }}>
            <Link className="btn" to="/internal/my-day">
              My day
            </Link>
      ...
      >
        <Stat
          cap="Total assignments"
          big={total}
          foot={`${Number(kpi.pending) || 0} pending · ${Number(kpi.in_flight) || 0} active`}
        />
        <Stat
          cap="In flight"
          big={Number(kpi.in_flight) || 0}
          foot={`${Number(kpi.pending) || 0} not started`}
          tone="b2"
        />
        <Stat
          cap="Completion"
          big={`${pct}%`}
          foot={`${done} of ${total} closed`}
          tone="b3"
        />
        <Stat
          cap="Overdue"
          big={overdue}
          foot={overdue ? "past due" : "nothing past due"}
          tone={overdue ? "b5" : "b4"}
        />
      </div>
      ...
      >
        <Card title="Status mix by department">
          <ChartBox h={240}>
            <StatusByDept by_status={summary.by_status} />
          </ChartBox>
        </Card>
          ...
          <ChartBox h={240}>
            <Doughnut
              data={{
                ...
                datasets: [
                  {
                    data: derived.prioCounts.some(Boolean)
                      ? derived.prioCounts
                      : [0, 0, 0, 1],
      ...
      <div
        className="grid"
        style={{ gridTemplateColumns: "1.3fr 1fr", marginBottom: 14 }}
      >
        <Card
          title="Due-date runway"
          extra={<span className="eyebrow">Next 14 days</span>}
        >
          <ChartBox h={200}>
            <Bar
              data={{
                ...
                datasets: [
                  ...
                  {
                    label: "High / critical",
                    data: derived.runHigh,
                    backgroundColor: RAY[0],
                    borderRadius: 3,
                  },
                ],
              }}
      ...
      <Card
        ...
        extra={<span className="eyebrow">Six months</span>}
      >
        <ChartBox h={200}>
          <Line
            data={{
              ...
              datasets: [
                {
                  ...
                  data: derived.cvcC,
                  borderColor: RAY[0],
                  backgroundColor: "rgba(35,64,142,.10)",
                  fill: true,
                  tension: 0.35,
                  pointRadius: 3,
                },
                {
                  ...
                  data: derived.cvcD,
                  borderColor: "#1DB5B6",
                  backgroundColor: "rgba(29,181,182,.12)",
                  fill: true,
                  tension: 0.35,
                  pointRadius: 3,
                },
      ...
      >
        <Card
          ...
          extra={
            <Link className="eyebrow" to="/internal/assignments">
              View all
            </Link>
          }
          pad={false}
        ...
        <Card title="Department load">
          {derived.deptLoad.length ? (
            derived.deptLoad.map((r, i) => (
              <div
                key={r.dp}
                style={{
                  display: "flex",
              ...
              >
                <span
                  className="mono"
                  style={{ fontSize: 11.5, color: "var(--muted)", width: 20 }}
                >
                ...
                <span
                  className="mono"
                  style={{ fontSize: 11.5, width: 24, textAlign: "right" }}
                >
                  {r.total}
                </span>
              </div>
            ...
            <p style={{ fontSize: 13, color: "var(--muted)", margin: 0 }}>
              No work assigned to any department yet.
            </p>
          )}
          <div style={{ marginTop: 8, fontSize: 11, color: "var(--muted)" }}>
            ...
            <span
              className="legend-dot"
              style={{ background: "var(--red)", marginLeft: 8 }}
            />
            overdue
            <span
              className="legend-dot"
      ...
      <div className="grid" style={{ gridTemplateColumns: "1fr 1fr" }}>
                ...
                >
                  <div>
                    ...
                    <div style={{ fontSize: 11.5, color: "var(--muted)" }}>
                      {t.department || "—"} · {t.assigned_to_name}
                    </div>
                  </div>
                  <span
                    className={`chip ${dl < 0 ? "down" : dl <= 2 ? "flat" : "up"}`}
                  >
                    {dl < 0
                      ? `${Math.abs(dl)}d late`
                      : dl === 0
        ...
        <Card
          ...
          extra={
            <Link className="eyebrow" to="/internal/meetings">
              View all
            </Link>
          }
        >
...
function StatusByDept({ by_status }) {
  ...
  const depts = [
    ...new Set(by_status.map((r) => r.department || "Unassigned")),
  ];
  if (!depts.length)
    return <p style={{ color: "var(--muted)" }}>No assignments yet.</p>;
  return (
    <Bar
      data={{
        labels: depts,
        datasets: STATUSES.map((s, i) => ({
          ...
          data: depts.map((dp) =>
            by_status
              .filter(
                (r) => (r.department || "Unassigned") === dp && r.status === s,
              )
